...
def findSpeciationPathFromTable(path_table, row_node_corr,count, xpos, ypos):
	...
	if(row_node_corr[xpos].is_leaf() and (ypos<0 or path_table[xpos, ypos] is None)):
		...
		if 's' in path_table[xpos, ypos]:
			...
			chemin.extend([",".join([row_node_corr[xpos].name+':%i'%(ypos+1),path1, path2]) for path1 in spec_1 for path2 in spec_2])
			...
			if c=='d':
				...
				chemin.extend([",".join([row_node_corr[xpos].name+':%i'%(ypos+1), path1]) for path1 in dup])
...
if __name__ == '__main__':
	...
	#"""
	genetree, specietree, distance_matrix, node_order = TreeUtils.polySolverPreprocessing('testinput/nostar_genetree.tree', 'testinput/speciestree.newick', 'testinput/nostar_famille_1.dist', capitalize=True, gene_sep = None)
