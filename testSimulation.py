...
def runTest(outfile, basedir, align_type, smap, specietree, alignfile, mltree, phylogeny, slimit=-1, plimit=-1, reroot='best', seuil=[100], algo='nj'):
	...
	with open("%s_treefix.csv"%outfile, 'a+') as OUT:
		header=['tree', 'leaves', 'TruePhylo dlc',  'TruePhylo p-val','TruePhylo dinl', 'Raxml dlc', 'Raxml rf', 'Raxml maxrf', 'Treefix dlc', 'Treefix rf', 'Treefix maxrf', 'Treefix p-val', 'Treefix dinl', 'Treefix time']
		headerWrite(OUT, header)
		treeCompute(OUT, rooted_tree, smap, specietree, alignfile, mltree_ext, phylogeny, data_size)


	header=['tree', 'PolySolver dlc', 'PolySolver rf','PolySolver maxrf', 'PolySolver dinl','PolySolver p-val',  'PolySolver time', 'PhyML time', 'total polytomy', 'polysolver nsol', 'best sol aLRT', 'is_bestrf', 'best_rf']
	...
	for thres in seuil:
		...
		with open("%s%s_polytomysolver.csv"%(outfile, thres), 'a+') as PSOUT:
			...
			pscompute(PSOUT, rooted_tree, smap, specietree, alignfile, gtree, thres, mltree_ext, reroot, slimit, plimit, phylogeny, polytomy_number, algo)
...
def pscompute(outfile, mltree, smap, specietree, alignfile, gtree, seuil, mltree_ext, r_option, slimit, plimit, correctPhylo, polytomy_number, algo):
	...
	n_psol, m_cost=fix_ps_out(ps_out)
	...
	with open(os.path.join(w_dir, basename+".%s.polytomysolver.all"%seuil), 'w') as ps_outfile:
		...
		line= [basename, polysolver_rdl, polysolver_rf, polysolver_maxrf, polysolver_dinl,polysolver_pval, ps_time, phy_time, polytomy_number, n_psol, likelihoods[bestposition], bestposition==best_rf_pos, psrfs[best_rf_pos]]
		outfile.write("\t".join([str(val) for val in line])+"\n")


def treeCompute(outfile, mltree, smap, specietree, alignfile, mltree_ext, correctPhylo, datasize):
	...
	with open(os.path.join(w_dir, basename+".tf.ml"), 'r') as tfml,  open(os.path.join(w_dir, basename+".tt.ml"), 'r') as ttml, open(os.path.join(w_dir, basename+".treefix.output"), 'r') as tfr, open(os.path.join(w_dir, basename+".true.output"), 'r') as ttr, open(os.path.join(w_dir, basename+".raxml.output"), 'r') as rxr:
		...
		#rf score
		raxml_rf, raxml_maxrf= getRFval(correctPhylo, mltree, unroot=False)
		...
		line=[basename, datasize, default_rdl, default_dinl, default_pval, raxml_rdl, raxml_rf, raxml_maxrf, treefix_rdl, treefix_rf, treefix_maxrf,treefix_dinl, treefix_pval, tf_time]
		outfile.write("\t".join([str(val) for val in line])+"\n")
