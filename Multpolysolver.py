...
def polySolver(genetree, specietree, gene_matrix, node_order, limit=-1, verbose=0, cluster_method='upgma'):
	...
	for path in paths:
		if(limit>0 and i>limit):
			break
		solution.append(constructFromPath(path, genetree, specietree, numpy.copy(gene_matrix), node_order[:], 1e305, method=cluster_method))
		i+=1
...
def findSpeciationPathFromTable(path_table, row_node_corr,count, xpos, ypos):
	...
	if(row_node_corr[xpos].is_leaf() and (ypos<0 or path_table[xpos, ypos] is None)):
		...
		if 's' in path_table[xpos, ypos]:
			...
			nb_node = count[snode]
			spec_1= findSpeciationPathFromTable(path_table, row_node_corr, count, spec_pos_1, ypos-nb_node)
			spec_2= findSpeciationPathFromTable(path_table, row_node_corr, count, spec_pos_2, ypos-nb_node)
			...
			if c=='d':
				dup=findSpeciationPathFromTable(path_table, row_node_corr,count, xpos, ypos+1)
...
def constructFromPath(chemin, genetree, specietree, gene_matrix, node_order, maxVal, verbose=0, method='upgma'):
...
if __name__ == '__main__':
	...
	corrected_tree= TreeClass("test/famille_1.corrected")
	c_tree= TreeClass("test/famille_1.corrected")
	...
	arbre_species= TreeClass('test/speciestree.newick')
	...
	#"""
	genetree, specietree, distance_matrix, node_order = TreeUtils.polySolverPreprocessing('test/nostar_genetree.tree', 'test/speciestree.newick', 'test/nostar_famille_1.dist', capitalize=True, gene_sep = None)
	polysolution = solvePolytomy(genetree, specietree, distance_matrix, node_order, poly_limit=2, method='nj')
