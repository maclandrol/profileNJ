...
class Solver(object):
    ...
    def get_solution(self, image_tree, multiplicities, reverse_node_map, outgene, ingene):
        ...
        nodemap = dict((node.name, node) for node in image_tree.traverse("postorder"))
        resolution = simpleConstruct(nodelist, nodemap, ingene, outgene, multiplicities, reverse_node_map, image_tree.name)
    ...
    def compute_table(self, image_tree, W, rW):
        ...
        def update(a, b, degree_diff):
            if degree_diff > 2:
                ...
                raise ValueError("Level difference incorrect, something is wrong")
    ...
    def compute_table(self, image_tree, W, rW):
        ...
        for node in image_tree.traverse("postorder"):
            ...
            if node.is_leaf():
                ...
                raise ValueError("Shouldn't have a polytomy in the image tree, something wrong")
        ...
        for node in image_tree.traverse("postorder"):
            ...
            if node.is_leaf():
                ...
                I[node.name] = np.zeros(M[node.parent]-W[node.parent]+1, dtype=int)
    ...
    def Ascend(self, node, maxW, W):
        ...
        if node.is_leaf():
            for i in xrange(1, maxW+1):
                self.mincost[node.name][i-1] = self.dupcost*max(mv-i, 0) + self.losscost*max(i-mv, 0)
        else:
            ...
            for i in xrange(1, maxW+1):
                for j in xrange(1, maxW+1):
                    self.cost[node.name][i-1, j-1] = self.dupcost*max(j-i+mv, 0) + self.losscost*max(i-mv-j, 0) + self.mincost[lchild.name][j-1] + self.mincost[rchild.name][j-1]
            for i in xrange(1, maxW+1):
