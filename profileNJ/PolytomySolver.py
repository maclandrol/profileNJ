...
from profileNJ.TreeLib import TreeUtils, TreeClass
...
class PolytomySolver:
  ...
  def computeCostsTable(self):
      ...
      dpsize = len(self.polytomy.get_children()) + 1
      dp_values = {}
      ...
      for s in self.speciestree.traverse("postorder"):
        ...
        if(s.is_leaf()):
            ...
            cl = self.cup_values[s.get_children()[0]]
            cr = self.cup_values[s.get_children()[1]]
            ...
            if self.use_dp:
              ...
              #first pass : compute values from above (when they exist)
              for k in range(1, dpsize):
                
                if k >= mult + 1:
                  dp_values[s][k] = dp_values[s1][k - mult] + dp_values[s2][k - mult]
                  if dp_values[s][k] < bmin:
                    bmin = dp_values[s][k]
                else:
                  dp_values[s][k] = MAXINT

              #second pass : locate the minimums on the row -> bb1 and bb2 are their positions
              for k in range(1, dpsize):
                if bb1 == MAXINT:
                  if dp_values[s][k] == bmin:
                    bb1 = k
              ...
              for k in range(1, bb1):
                dp_values[s][bb1 - k] = min(dp_values[s][bb1 - k], dp_values[s][bb1 - k + 1] + s_dupcost)
              for k in range(bb2 + 1, dpsize):
                dp_values[s][k] = min(dp_values[s][k], dp_values[s][k - 1] + s_losscost)
            #---------------------------------------------------
  ...
  def getResolutions(self, s, k, limit = 1):
    ...
    if s.is_leaf():
      ...
      #TODO : we just take one way of doing this
        ...
        #NOTE the [0] which assumes that there's only one possible k+1 resolution
      ...
      #get k - 1 guys, add a loss
        ...
        #NOTE the [0] which assumes that there's only one k-1 resolution
        resz = self.getResolutions(s, k - 1, 1)[0]
    ...
    #------------------------------------------
        ...
        for s1_iter in range(0, len(all_resz_s1)):            
          ...
          for s2_iter in range(0, len(all_resz_s2)):
            ...
            if limit > 0:
              ...
              for i in range(0, k):
                if i <= k - mult - 1:
                    if len(resz_s1) > i and len(resz_s2) > i:   #this can happen when losses were inserted
                    ...
                    #we go here when a child of the polytomy was internal and had species s (mult > 0)
                    resz.append(s.name)
      ...
      #The duplication case.  We take the first two subtrees from the right and join them
...
class GeneTreeSolver:
  ...
  """ Solves each polytomy, combines each solution and returns an array of newick strings.
    ...
    s_images = TreeUtils.getImageTreeNode(self.genetree, self.speciestree, self.lcaMapping)
    ...
    for g in self.genetree.traverse("postorder"):
      ...
      if not g.is_leaf():
        ...
        for special_sp in special_cost_species:
          ps.setSpecialDupLossCosts(special_sp, self.dupcost, self.losscost * special_cost_species[special_sp])
        
        ps.debug = self.debug
        ps.setDupLossCosts(self.dupcost, self.losscost)
        ps.use_dp = (self.use_dp or len(special_cost_species) > 0)
        ps.computeCostsTable()

        if self.debug:
          print "COST=", ps.getTableValue(self.speciestree, 1)

        r = ps.getAllResolutions(limit)
        
        #TODO : limit is not applied so nicely here
        #TODO : handle the case ( (a,a,a), b, b ) -> here a,a,a makes a leaf label
        for gchild in g.get_children():
          ...
          #In this manner, the internal children of r get replaced incrementally.
