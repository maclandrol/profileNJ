...
class TestTreeClass(unittest.TestCase):
	...
	def test_get_nodes_between(self):
		...
		node2 = self.tree2&'c'
		t_nodes_between = 'ef'
		computed_nodes_between = "".join([n.name for n in node1.get_nodes_between(node2)])
	...
	def test_reroot(self):
		...
		for i in xrange(len(rerooted)):
			for j in xrange(i, len(rerooted)):
				rf = rerooted[i].robinson_foulds(rerooted[j], unrooted_trees=True)[0]
				assert rf == 0
		for i in xrange(len(edge_reroot)):
			for j in xrange(i, len(edge_reroot)):
				rf = edge_reroot[i].robinson_foulds(edge_reroot[j], unrooted_trees=True)[0]
				assert rf == 0
