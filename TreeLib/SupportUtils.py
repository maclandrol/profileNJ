...
def runConsel(basedir, sequence, trees, n_sol):
	phyml = _Phyml.PhymlCommandline(cmd="phyml", input=sequence, input_tree=trees , optimize="none", bootstrap=0, print_site_lnl=True)
	phyml()
		...
		for line in CONSELOUT:
			if(line.startswith('#')):
				if first and 'reading' not in line and ("%s+"%sort not in line):
					title.extend(line.replace('#', '').replace('|', '').strip().split())
					first=False
				elif not first:
	...
	if not out:
		...
		with open(out, 'w') as fout:
			for file in listfile:
				print >>fout, open(file, 'r').read().strip().replace('\s','').replace('\n', '')

	#cleaning for raxml
	for file in glob.glob("%s/*trees"%basedir):
		os.remove(file)

	cmd="raxmlHPC-SSE3 -f g -z %s -s %s -m GTRGAMMA -n trees -w %s" %(out, alignfile, os.path.abspath(basedir))
